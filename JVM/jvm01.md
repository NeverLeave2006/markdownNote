...
作用:记住下一条jvm指令的执行地址,在CPU中用寄存器实现
特点:
   - 线程私有，每个线程有一个对应的程序计数器  
   - 唯一不会内存溢出的区
...
2. Young Collection + Collection Mark
