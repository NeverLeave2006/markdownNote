...
CAS即Compare and Swap,它体现了一种乐观锁,比如多个线程要对一个共享的整型变量执行+1操作:
...
while(true){
    ...
    if(cimpareAndSwap(old,res)){
        //成功,退出循环
    }
}
```

获取共享变量时，为了保证该变量的可见性,需要使用volatile修饰。结合CAS和volatile可以实现无所并发，适用于竞争不激烈，多核CPU的场景下

- 因为没有使用synchronized,所以线程不会陷入组在啊，这是效率提升的因素之一
- 但如果竞争激烈，可以想到重试必然平凡发生，反而效率会受影响

CAS底层依赖于一个Unsafe类来直接调用操作系统的CAS指令，下面是直接使用Unsafe对象进行1线程安全保护的一个例子

4.2 乐观锁和悲观锁

- CAS是基于乐观锁的思想: 最乐观的估计，不怕别的线程来修改共享变量，就算改了也没关系，我吃亏点再重试
- synchronized是基于悲观锁的思想: 最悲观的估计，得防着其他线程来修改共享变量，我上了锁匿名都别想改，我改完了揭开所，匿名才有机会

4.3 原子操作类

JUC提供了原子操作类，可以提供线程安全的操作，例如： AtomicInteger，AtomicBoolean等，它们底层就是采用CAS技术+volatile来实现的

# 5 Synchronizedy优化

Java HotSpot虚拟机中，每个对象都有对象头(包括class指针和Mark Word)。Mark Word平时存储这个对象的哈希码，分带年龄，当加锁时，这些信息就会根据情况被替换为标记位，线程锁记录指针，重量级锁指针，线程ID等内容
...
如果这期间有其他学生(线程B)来了,会告知(线程A)有并发访问，线A随机升级为重量级锁，进入重量级锁的流程

而重量级锁就不是那么用可悲呢占座那么简单了,可以想象线程A走之前，把座位用一个铁栅栏围起来

假设有两个方法同步块，利用同一个对象加锁

每个线程的栈帧都会包含一个锁记录的结构，内部可以存储锁定对象Mark Word

|线程1|对象Mark Word|线程2|
|----|----|----|
|访问同步块A,把Mark复制到线程1的所记录|01(无锁)|-|
|CAS修改Mark为线程1锁记录地址|01(无锁)|-|
|成功(加锁)|00(轻量锁)线程1锁记录地址|-|
|执行同步块A|00(轻量锁)线程1锁记录地址|-|
|访问同步块B,把Mark复制到线程1的所记录|00(轻量锁)线程1锁记录地址|-|
...
|-|00(轻量锁)现场2锁记录地址|成功(加锁)|


5.2 锁膨胀

尝试在加轻量级锁的过程中，CAS无法成功,这时一种情况就是有其他线程为此对象加上了轻量级锁(有竞争)，这时需要进行锁膨胀，将轻量级锁变为重量级锁

5.3 重量锁

重量级锁竞争的时候，还可以使用自选来优化，如果当前现场自旋成功(即这时候持锁线程已经推出了同步块，释放了锁)，这时当前线程就可以避免阻塞。

在Java6之后自旋锁是自适应的，比如对象刚刚的一次自旋操作成功过，那么认为这次自旋操作成功的可能性会高。就多自旋几次，反之，就少自旋甚至不自旋，总之，比较智能。

- 自旋会占用CPU时间,单核CPU自旋就是浪费,多核CPU自旋才能发挥优势
- 好比等红绿灯汽车不是熄火,不熄火相当于自旋(等待时间短了划算),熄火了相当于阻塞(等待时间长了划算)

- java 7之后不能控制是否开启自旋功能

5.2 偏向锁

轻量级锁在没有竞争时(就自己这个线程),每次重入仍然需要执行CAS操作。Java6中引入了偏向锁来做进一步优化:
只有第一次使用CAS将线程ID设置到对象的Mark Word头,之后发现这个线程ID是自己的就表示没有竞争,不用重新CAS
- 撤销偏向需要将持锁线程升级为轻量级锁,这个过程中所有线程需要暂停(STW)
- 访问对象的hashCode也会撤销偏向锁
- 如果对象虽然被多个线程访问,但没有竞争，这时偏向了T1的对象仍有机会重新偏向T2，重偏向会从之对象的ThreadId
- 撤销偏向和重偏向都是批量进行的以类为单位
- 如果撤销偏向达到某个阈值,整个类的所有对象都会变为不可偏向的
- 可以主动使用-XX:-UseBiasedLocing禁用偏向锁
...
2. 减少锁的粒度
   将一个锁拆分为多个锁提高并发度,例如:
   - ConcurrentHashMap
   - LongAdder分为base和Cells两部分。没有并发竞争用的时候或者是Cells数组正在初始化的时候，会用CAS来累加值到base,有并发争用，会初始化cells数组，数组有多个cell,基于运行有多少线程并行修改，最后将数组中每个cell累加,再加上base就是最终的值
   - LinkBlockingQueeu入队和出队使用不同的锁，相对于LinkedBlockingArray只有一个锁效率要高
3. 锁粗化
多次循环inroad铜鼓块不如同步块内多次循环
另外JVM可能会做如下优化，把多次append的加锁操作粗话为一次(因为都是对同一个对象加锁，没必要重入多次)

4. 锁消除
JVM会进行代码的逃逸分析,例如某个加锁对象是方法内部变量，不会被其他线程锁访问带，这时候就会被即时编译器忽略掉所有同步操作
